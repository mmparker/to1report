...
# xtable() adds attributes to the data.frame to make it more suitable for display
...
dfprint <- function(df) {
    print(
        xtable(format(df, na.encode = FALSE), 
...
weeks_left <- new_interval(Sys.Date(), local_facts$period_end) / duration(num = 1, units = "weeks")
...
### Test Results
```{r,echo=FALSE,results='asis',message=FALSE, warning=FALSE,error=FALSE}
# Summarize participant test results
...
# MATT: make two separate factors - one for sorting by name, one for sorting by proportion.
# Use the first for the table, second for the plot

```

```{r,echo=FALSE, results="hide",message=FALSE, warning=FALSE,error=FALSE,fig.width = 8, fig.height = 8}

ggplot(testagg, aes(x = as.character(Results), weight = Freq)) +
  geom_bar() +
  coord_flip()
...
# Pretty names
names(tripnegtoclose) <- c("StudyId", "Status", "Visit Date", "TST Neg?", "QFT Neg?",
...
# Pretty names
names(missingtest) <- c("StudyId", "Visit Date", "TST Result", "QFT Result", "TSPOT Result")
